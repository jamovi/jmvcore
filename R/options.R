...
Option <- R6::R6Class(
    ...
    private=list(
        ...
        .check=function(data){}),
    public=list(
        initialize=function(parent, name, value=NULL, ...) {
...
OptionGroup <- R6Class(
    ...
    inherit=Option,
    public=list(
        initialize=function(parent, name, value, elements, ...) {
            private$.elements <- list()
            for (element in elements)
                private$.elements[[element$name]] <- element
            super$initialize(parent, name, value, ...)
        }
    ),
    active=list(
        value=function(value) {
...
OptionArray <- R6Class(
    ...
    inherit=Option,
    public=list(
        initialize=function(parent, name, value, template, ...) {
    ...
    active=list(
        value=function(values) {
            if (base::missing(values)) {
                values <- list()
                for (o in private$.elements)
                    values[[length(values)+1]] <- o$value
                return(values)
            }
            private$.elements <- list()
            for (value in values) {
