...
SEXP deepCloneRecurse(SEXP obj, Clones &clones) {

    Clones::iterator itr = clones.find(obj);
    if (itr != clones.end())
        return itr->second;
    ...
    if (is<Environment>(obj)) {
            ...
            if (isR6 && (nameStr) == ".__enclos_env__") {
                ...
                if (priv != R_NilValue) {
                    ...
                    for (SEXP pname : pnames) {
                        SEXP pchild = deepCloneRecurse(penv.get(pname), clones);
                ...
                if (R_BindingIsActive(sym, obj)) {
                    ...
                    SEXP child = deepCloneRecurse(old.get(name), clones);
        ...
        for (int i = 0; i < nu.size(); i++) {
            ...
            if (is<Environment>(child))
                nu[i] = deepCloneRecurse(child, clones);
        }
        return nu;
    }
    else {
...
SEXP deepClone(SEXP obj) {
    Clones clones;
    return deepCloneRecurse(obj, clones);
}

// [[Rcpp::export]]
SEXP repR6(Environment obj, int n) {
    List lst(n);
